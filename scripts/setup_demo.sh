...
if [ -n "$FUNNEL_URL" ]; then
    ...
    echo "Next steps:"
    echo ""
    echo "  1. Start the MCP server (new terminal):"
    echo "     uv run python mcp_server/server.py"
    echo ""
    echo "  2. Expose MCP via Tailscale Funnel:"
    echo "     tailscale funnel --set-path /mcp 8080"
    echo ""
    echo "  3. Test MCP pipeline:"
    echo "     uv run python scripts/test_mcp_agent.py --url ${FUNNEL_URL}/mcp"
    echo ""
    echo "  4. Configure OpenHands Cloud (Settings → MCP):"
    echo "     ${FUNNEL_URL}/mcp"
    echo ""
    echo "  5. Create a demo issue:"
    ...
    echo "     tailscale funnel --set-path / 15000     # demo service"
    echo "     tailscale funnel --set-path /mcp 8080   # MCP server"
    echo ""
    echo "For local-only testing:"
    echo "  uv run python mcp_server/server.py          # start MCP"
    echo "  uv run python scripts/test_mcp_agent.py     # test pipeline"
fi
echo ""
echo "Quick commands:"
echo "  Break service1:   docker exec openhands-gepa-demo touch /tmp/service.lock"
echo "  Fix (MCP):        uv run python scripts/test_mcp_agent.py"
echo "  Fix (manual):     ./scripts/fix_demo.sh service1"
echo ""
